...
flags.DEFINE_integer('noniid_level', 100, 'Level of noniid.')
flags.DEFINE_integer('N', 20,
                   'Total number of clients.')
flags.DEFINE_integer('max_steps', 1000,
                   'Total number of communication round.')
flags.DEFINE_integer('local_steps', 10,
...
flags.DEFINE_integer('num_microbatches', 16, 'Number of microbatches '
...
flags.DEFINE_string('eps', 'gauss safety level', 'epsilon file name.')
...
flags.DEFINE_float('sample_ratio', 1.0, 'Sample ratio.')

# minimum epsilon
flags.DEFINE_boolean('min', False, 'If True, train eps_min dp.')
# weighted average
flags.DEFINE_boolean('weiavg', True, 'If True, train with weighted averaging.')
# fedavg
...
# save dir flags
flags.DEFINE_integer('version', 3, 'version of dataset.')
...
def prepare_local_data(project_path, dataset, nclients, noniid, version):
    ...
    print('数据集保存地址：{}'.format(data_path))
    # universal set
    ...
    # split the universal
    client_set_path = os.path.join(data_path, dataset, 'clients', ('noniid' if noniid else 'iid'), 'v{}'.format(version))

    client_dataset_size = len(x_train) // nclients if FLAGS.client_dataset_size is None else FLAGS.client_dataset_size
    if not noniid:
        ...
        client_set = create_clients.create_noniid_clients(y_train, nclients, 10,
                                                          FLAGS.noniid_level, client_set_path)
...
def main(unused_argv):
    ...
    for i in range(5):
        ...
        print('项目地址：{}'.format(project_path))
        # prepare the local dataset all clients
        x_train, y_train, x_test, y_test, client_set = prepare_local_data(project_path, FLAGS.dataset, FLAGS.N, FLAGS.noniid, FLAGS.version)
                ...
                for r in range(COMM_ROUND):
                        ...
                        for cid in participants:
                            ...
                            if FLAGS.dpsgd:
                                print('For client %d and delta=%f, the budget is %f and the left budget is: %f' %
                                    (cid, delta, clients[cid].ba.epsilon, clients[cid].ba.epsilon - clients[cid].ba.accum_bgts))
                    ...
                    if FLAGS.dpsgd:
                        ...
                        if FLAGS.delay:
                            ...
                            main_utils.save_accuracy(FLAGS, model, accuracy_accountant, i, privacy_accountant, accum_nbytes_list1, accum_nbytes_list2)
                            main_utils.save_loss(FLAGS, model, loss_accountant, i, privacy_accountant, accum_nbytes_list1, accum_nbytes_list2)
                        else:
                            main_utils.save_accuracy(FLAGS, model, accuracy_accountant, i, privacy_accountant)
                            main_utils.save_loss(FLAGS, model, loss_accountant, i, privacy_accountant)

                    else:
                        main_utils.save_accuracy(FLAGS, model, accuracy_accountant, i)
                        main_utils.save_loss(FLAGS, model, loss_accountant, i)
