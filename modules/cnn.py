...
class CNN(Model):
    ...
    def evaluation(self, logits, labels):
        ...
        # logits：模型的预测输出，通常是未经处理的得分（logits），形状为 [batch_size, num_classes]。
        # labels：实际标签，形状为 [batch_size]，其中每个元素是实际类别的索引。

        # 这个操作返回一个布尔张量，形状为 [batch_size]，每个元素表示对应样本的预测是否正确（即，预测的最可能类别是否与实际标签相同）。
        correct = tf.nn.in_top_k(logits, labels, 1)
        # tf.cast(correct, tf.int32)：将布尔张量 correct 转换为整型张量，其中 True 转换为 1，False 转换为 0。
        # tf.reduce_sum(tf.cast(correct, tf.int32))：对转换后的整型张量进行求和，得到正确预测的样本数。
        return tf.reduce_sum(tf.cast(correct, tf.int32))
    

    def init_placeholder(self):
        # 获取图像尺寸img_size：（宽、高、通道数）。 此处mnist为（28,28,1）
        img_size = IMAGE_SIZE[self.dataset]
        # 获取图像像素数img_pixels：（宽 × 高 × 通道数）。 此处mnist为28x28x1=784
