...
class ServerOperation(metaclass=abc.ABCMeta):
    ...
    def _add_one(self, num_vars, _update, _agg_updates):
        if not len(_agg_updates):
            return [np.expand_dims(_update[i], 0) for i in range(num_vars)]
        else:
            return [np.append(_agg_updates[i], np.expand_dims(_update[i], 0), 0) for i in range(num_vars)]
...
class FedAvg(ServerOperation):
    ...
    def average(self, num_vars=None, shape_vars=None, eps_subset=None):
        mean_updates = [np.average(self.__updates[i], 0).reshape(shape_vars[i]) for i in range(num_vars)]
        self.__updates = []
        return mean_updates
...
class WeiAvg(ServerOperation):
    ...
    def average(self, num_vars=None, shape_vars=None, eps_subset=None):
        ...
        mean_updates = [np.average(self.__updates[i], 0, weights).reshape(shape_vars[i]) for i in range(num_vars)]
        self.__updates = []
        return mean_updates
    ...
    def __projection(self, num_vars, shape_vars):
        ...
        if len(self.__priv_updates):
            ...
            for i in range(num_vars):
                ...
                mean_updates[i] = (
                            (self.__num_priv * mean_proj_priv_updates[i] + self.__num_pub * mean_pub_updates[i]) /
    ...
    def __weighted_project_priv_updates(self, num_vars, shape_vars):
        ...
        if len(self.__priv_updates):
            ...
            for i in range(num_vars):
                ...
                mean_proj_priv_updates[i] = np.dot(Vk, np.dot(Vk.T, (mean_priv_updates[i] - mean))) + mean
                mean_updates[i] = ((mean_proj_priv_updates[i] * sum(self.__priv_eps) + mean_pub_updates[i] * sum(
                    self.__pub_eps))
        ...
        if len(self.__priv_updates):
            ...
            if warmup:
                for i in range(num_vars):
                    ...
                    mean_proj_priv_updates[i] = np.dot(Vk, np.dot(Vk.T, (mean_priv_updates[i] - mean))) + mean
                    mean_updates[i] = ((sum(self.__priv_eps) * mean_proj_priv_updates[i] + sum(self.__pub_eps) *
                                        mean_pub_updates[i]) /
