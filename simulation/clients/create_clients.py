...
def create_noniid_clients(targets, num_clients, num_classes, dir_alpha, path, min_require_size=None):
    file_path = os.path.join(path, '{}_{}_clients.pkl'.format(num_clients, dir_alpha))
    if os.path.exists(os.path.join(file_path)):
        print('Client exists at: {}'.format(file_path))
        client_set = pickle.load(open(file_path, 'rb'))
        return client_set
    ...
    for cid in range(num_clients):
        client_set.append(idx_batch[cid])
